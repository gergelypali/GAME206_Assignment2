...
{
	// in editor mode draw the debuglines as the patrolpath
	FlushPersistentDebugLines(GetWorld());
	if (PatrolPath.Num() > 1)
	{
		DrawDebugDirectionalArrow(GetWorld(), PatrolPath.Last() + GetTransform().GetLocation(), PatrolPath[0] + GetTransform().GetLocation(), 120.0f, FColor::Magenta, true, -1, 0, 0.5f);
		for (int i = PatrolPath.Num() - 1; i > 0; --i)
		{
			DrawDebugDirectionalArrow(GetWorld(), PatrolPath[i - 1] + GetTransform().GetLocation(), PatrolPath[i] + GetTransform().GetLocation(), 120.0f, FColor::Magenta, true, -1, 0, 0.5f);
		}
	}
}
...
{
	Super::BeginPlay();
	for (FVector loc : PatrolPath)
	{
		PatrolPathOnBegin.Add(loc + GetTransform().GetLocation());
	}
}
